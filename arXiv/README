...
- Numerically verify the optimal contraction factors for five algorithms using CVX
  - Gradient Descent with Exact Line Search (GDELS_CVX.m)
  - Gradient Descent with Armijo Rule (GD_armijo_CVX.m)
  - Gradient Descent with Constant Step Size (GD_const_CVX.m)
...
- Numerically verify the optimal contraction factors for the same five algorithms using YALMIP (*_YALMIP.m)
- Symbolically verify that the set of linear equalities hold for all five algorithms (*_verify_lin_eq.m)
- Symbolically verify that the matrix Q is positive semidefinite for all five algorithms (*_verify_Q_PSD.nb)
